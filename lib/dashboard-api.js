...
export async function getLatestReadings(thresholds) {
  if (isConfigured) {
    ...
    return parseLatest(raw, thresholds);
  }
    ...
    const res = await fetch(`${API_URL}/command`, {
      ...
      body: JSON.stringify({
        ...
        thresholds: {
          ...
          temp_danger_threshold: thresholds.temperature.danger,
          humidity_min:          thresholds.humidity.min,
          humidity_max:          thresholds.humidity.max,
          low_stock_weight:      thresholds.weight.low_stock,
          tvoc_fan_threshold:    thresholds.tvoc.fan_threshold,
          tvoc_buzzer_threshold: thresholds.tvoc.buzzer_threshold,
          light_threshold:       thresholds.light.threshold,
        },
      }),
    });
    if (!res.ok) throw new Error(`API error: ${res.status}`);
...
// Fetch raw history records with all fields (alert, actuator states, etc.)
export async function getRawHistory(range = "7d") {
  if (!isConfigured) return [];
  try {
    const res = await fetch(`${API_URL}/history?device_id=${DEVICE_ID}&type=temperature&range=${range}`, {
      headers: { "x-api-key": API_KEY },
    });
    if (!res.ok) throw new Error(`API error: ${res.status}`);
    const raw = await res.json();
    ...
    return items
      .map(item => {
        ...
        return {
          ...
          temperature: parseFloat(p.temperature ?? NaN),
          humidity:    parseFloat(p.humidity    ?? NaN),
          weight:      parseFloat(p.weight      ?? NaN),
          tvoc:        parseInt(p.tvoc_level    ?? -1),
          alert:    p.alert    === 1,
          badAir:   p.badAir   === 1,
          lowStock: p.lowStock === 1,
          fan:      p.fan      === 1,
          led:      p.led      === 1,
          buzzer:   p.buzzer   === 1,
        };
...
function parseLatest(raw, thresholds) {
  const d = thresholds || getDefaultThresholds();
  ...
  if (typeof ts === "number" || !isNaN(Number(ts))) {
    ...
    // Convert seconds → milliseconds if timestamp looks like Unix seconds (10-digit)
  ...
  return {
    ...
    freshness: {
      ...
      brownIndex: parseFloat(raw.brownIndex ?? 0),
    },
  };
}
...
export function getDefaultThresholds() {
  return {
    temperature: { min: 0,   max: 8,    danger: 40  },
    humidity:    { min: 85,  max: 95                 },
    weight:      { low_stock: 500                    },
    tvoc:        { fan_threshold: 3, buzzer_threshold: 4 },
    light:       { threshold: 5                      },
    freshness:   { aging_threshold: 0.08, spoiled_threshold: 0.15 },
  };
}
